...
/* we can probably optimize this by not using 8 chars to encode one byte */
/* (see masking...) */
...
static unsigned char bytesub(unsigned char c)
{
  ...
  c = invert(c);
  for (i = 0;i < 8;++i) f[i] = 1 & (c >> i);
  g[0] = f[0] ^ f[4] ^ f[5] ^ f[6] ^ f[7] ^ 1;
  g[1] = f[1] ^ f[5] ^ f[6] ^ f[7] ^ f[0] ^ 1;
  g[2] = f[2] ^ f[6] ^ f[7] ^ f[0] ^ f[1];
  g[3] = f[3] ^ f[7] ^ f[0] ^ f[1] ^ f[2];
  g[4] = f[4] ^ f[0] ^ f[1] ^ f[2] ^ f[3];
  g[5] = f[5] ^ f[1] ^ f[2] ^ f[3] ^ f[4] ^ 1;
  g[6] = f[6] ^ f[2] ^ f[3] ^ f[4] ^ f[5] ^ 1;
  g[7] = f[7] ^ f[3] ^ f[4] ^ f[5] ^ f[6];
  c = 0;
  for (i = 0;i < 8;++i) c |= g[i] << i;
